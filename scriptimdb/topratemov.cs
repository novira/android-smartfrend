...
public class topratemov : MonoBehaviour {
	string key = "aa9ba31a0fc74a076849181a5b323377";
	public Button menu,next;
	public Button overview1,overview2,overview3;
	public Text title1, title2, title3;
	public Text genre1, genre2, genre3;
	public Text Release1,Release2,Release3;
	...
	void Start () {
		...
		overview1.onClick.AddListener (() => StartCoroutine (overview (0)));
		overview2.onClick.AddListener (() => StartCoroutine (overview (1)));
		overview3.onClick.AddListener (() => StartCoroutine (overview (2)));
	}
	...
	IEnumerator getplaymovie1() {
		UnityWebRequest www = UnityWebRequest.Get("https://api.themoviedb.org/3/movie/top_rated?api_key="+key+"&language=en-US&page=1");
		...
		if(www.isError) {
			...
			if (www1.isError) {
				...
				for (int i = 0; i < cb1.Count; i++){
					...
					if (item1.GetValue ("id").ToString () == result2["genre_ids"][0].ToString()) {
						genre3.text = item1.GetValue ("name").ToString ();
					}
